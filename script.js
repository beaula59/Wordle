  ...
  const startGame = async () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        //Disabled by default. Will be enabled one by one
  ...
  const checker = async (e) => {
    const keys=[..."abcdefghijklmnopqrstuvwxyz","Backspace"];
    ...
    if (value.length == 1) {
      //if the word is lesss than 5 in length and the button isn't backspace
  ...
  const validateWord = async () => {
    ...
    if (failed) {
      inputCount=0
      finalWord=""
      let start=inputRow[tryCount].firstChild
      for(let i=0;i<5;i++){
      start.value=""
      ...
      if (successCount == 5) {
        ...
        setTimeout(() => {
          ...
          winScreen.innerHTML = `
          <span class="result">You've WON!</span><br>
        ...
        if (tryCount == 6) {
          ...
          tryCount = 0;
          winScreen.classList.remove("hide");
          winScreen.innerHTML = `<span class="result">You've lost</span><br>
          <span>The word was ${randomWord}!</span>
            <button onclick="startGame()">New Game</button>`;
          return false;
        }
